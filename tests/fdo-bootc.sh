...
podman login --log-level=debug -u "${STAGE_REDHAT_IO_USERNAME}" -p "${STAGE_REDHAT_IO_TOKEN}" registry.stage.redhat.io
...
COPY fido-device-onboard-rs/rpmbuild/RPMS/x86_64/fdo-init*.rpm /tmp/
COPY fido-device-onboard-rs/rpmbuild/RPMS/x86_64/fdo-client*.rpm /tmp/
